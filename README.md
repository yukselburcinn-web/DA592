...
Writes `evaluation/comparative_analysis_results.csv` (per-query), `evaluation/comparative_analysis_summary.csv` (aggregated) and `evaluation/comparative_analysis_significance.csv` (the pairwise tests below). It scores 67 queries across 2 cities and 7 archetypes on five metrics: multi-hop recall against an independent Wikivoyage gold set, archetype precision, structural grounding rate, itinerary coherence (km per stop), and retrieval latency. *Structural grounding* asks whether retrieval returned a knowledge-base row at all — it is 1.0 by construction for anything that retrieves, and it is **not** a hallucination measurement however the previous name (`grounded_entity_rate`) read. The hallucination rate itself is measured separately, against generated text, by `evaluation/hallucination.py` ([`#132`](https://github.com/yukselburcinn-web/DA592/issues/132)): over 106 generations on NVIDIA's `nemotron-3.5-lightning-30b-a3b`, the **geographical hallucination rate is 0.0 for all three configurations** (no narrative named a place from the other city, across 1,443 place mentions), while 4.8% / 3.3% / 3.3% of named places were real, in the right city, and never shown to the model. See [`REPORT.md` §3.5](REPORT.md#geographical-hallucination-rate-issue-132) for why the first number is weaker evidence than it looks. Current summary:
...
Because a gap between two averages can be noise, each metric is also tested pairwise (Wilcoxon signed-rank across the same queries — see the **Is the lead real?** table in the app). Against Hybrid RAG, Fusion RAG is significantly better on **multi-hop recall** (p<0.0001), **archetype precision** (p<0.0001) and **km per stop** (p=0.0027), *tied by construction* on structural grounding, and slower by roughly 8 ms per retrieval. Recall separates only since the gold set stopped being computed from the graph the retriever itself walks; see [`REPORT.md` §3.5](REPORT.md#35-comparative-analysis) for why the earlier version of this table reported no difference.
