...
Open `http://localhost:8501`. The image ships the committed dataset (real POIs + real demand data, see [Data note](#data-note)) and pre-built knowledge graph, so no data-generation step is needed first. It also ships the committed evaluation results, which is what makes **System logs → Results** show the comparison rather than an empty tab — `.dockerignore` excluded them until a cleanup pass measured what that saved (536 KB against 49 MB) and what it cost (the screen).
...
Writes `evaluation/comparative_analysis_results.csv` (per-query), `evaluation/comparative_analysis_summary.csv` (aggregated) and `evaluation/comparative_analysis_significance.csv` (the pairwise tests below). It scores 68 queries across 2 cities and 7 archetypes on five metrics: multi-hop recall against an independent Wikivoyage gold set, archetype precision, structural grounding rate, itinerary coherence (km per stop), and retrieval latency. *Structural grounding* asks whether retrieval returned a knowledge-base row at all — it is 1.0 by construction for anything that retrieves, and it is **not** a hallucination measurement however the previous name (`grounded_entity_rate`) read. The hallucination rate itself is measured separately, against generated text, by `evaluation/hallucination.py` ([`#132`](https://github.com/yukselburcinn-web/DA592/issues/132)): over 109 generations on NVIDIA's `nemotron-3.5-lightning-30b-a3b`, the **geographical hallucination rate is 0.0 for all three configurations** (no narrative named a place from the other city, across 1,352 place mentions), while 2.4% / 1.3% / 1.4% of named places were real, in the right city, and never shown to the model. See [`REPORT.md` §3.5](REPORT.md#geographical-hallucination-rate-issue-132) for why the first number is weaker evidence than it looks. Current summary:

| Config | Multi-hop recall | Recall vs attainable | Archetype precision | Grounded | Km/stop | Latency |
|---|---|---|---|---|---|---|
| **Fusion RAG** | **0.231** | **0.517** | **0.967** | 1.000 | 0.91 | 20.1 ms |
| Hybrid RAG | 0.121 | 0.250 | 0.659 | 1.000 | 1.05 | 10.6 ms |
| Standard prompting | 0.000 | 0.000 | 0.438 | 0.000 | 0.78 | 0.0 ms |

Because a gap between two averages can be noise, each metric is also tested pairwise (Wilcoxon signed-rank across the same queries — see the **Is the lead real?** table in the app). Against Hybrid RAG, Fusion RAG is significantly better on **multi-hop recall** (p<0.0001), **archetype precision** (p<0.0001) and **km per stop** (p=0.0028), *tied by construction* on structural grounding, and slower by 9.6 ms per retrieval. Recall separates only since the gold set stopped being computed from the graph the retriever itself walks; see [`REPORT.md` §3.5](REPORT.md#35-comparative-analysis) for why the earlier version of this table reported no difference.
...
The queries come in three tiers, and the split is the point: **24** are hand-written the way a traveler would phrase a question, **34** are generated to sweep city × category cells evenly, and **10** ask what can be *sequenced* from where the day starts — the chain tier [`#126`](https://github.com/yukselburcinn-web/DA592/issues/126) added, which is the relation neither of the other two can express. Recall vs attainable by tier: grid 0.642, hand-written 0.437, chain 0.288 for Fusion, against 0.317 / 0.233 / 0.063 for Hybrid — the grid is the easy half and it is also the largest slice, which is why the headline is weighted by query count and why [`REPORT.md` §3.5](REPORT.md#35-comparative-analysis) also states what equal tier weighting would give. Both leads hold in **all three** tiers separately (archetype precision p=0.00033 / 0.000011 / 0.0039; recall p=0.00034 / 0.000012 / 0.016), so neither is a property of how the questions are phrased. This paragraph described two tiers and reported recall as indistinguishable until the cleanup pass that produced this revision: that was the pre-#126 query set, and it contradicted the paragraph above it. See [`#50`](https://github.com/yukselburcinn-web/DA592/issues/50) for how the query set is built and [`#48`](https://github.com/yukselburcinn-web/DA592/issues/48) for why the chain tier's answer key is graded under its own dependence level.
...
With the extra installed, **the LangGraph path is selectable from the app**: the sidebar's *Agentic orchestration* picker chooses which orchestrator plans the trip, the default stays the custom state machine, and the System logs screen names the one that ran (`orchestrator=custom` / `orchestrator=langgraph` on the first record of each plan). Without the extra the app runs exactly as before — the picker offers the custom path only and says why. Both orchestrators log the same steps under the same names, so the operator screen reads alike either way (issue #129).

It is also importable directly, which is how the evaluation scripts and tests reach it:
...
The catalogue covers **2 cities — Paris (371 POIs) and Berlin (283), 654 in all**. 400 and 300 are the *targets* `pipeline/common.CITIES` aims each build at, and this file quoted them as counts until the cleanup pass that produced this revision; what ships is what survived the filters, including the 46 rows [`#65`](https://github.com/yukselburcinn-web/DA592/issues/65) removed for not being places to visit. That is a deliberate trade of breadth for depth: the previous eight-city set carried 150 POIs each, and per-city depth is what the retrieval comparison and the itinerary optimiser actually exercise. Coverage against an independent Wikivoyage reference list roughly doubled as a result (Paris 38% → 74.4%, Berlin to 67.2%, measured on the same one-to-one matching).

- **POIs** (`pipeline/build_catalogue.py`): each city's points of interest are pulled from **Wikidata** (SPARQL, via the QLever endpoint) and cross-matched against **OpenStreetMap** (Overpass) within ~7km of the city centre, mapped onto RoamWise's ten-category taxonomy. Candidates are ranked by a fame score blending Wikidata sitelink count with Wikipedia pageviews (weights fitted against the reference list), so a city's catalogue is its best-known sights rather than an arbitrary sample of its tagged features. Entities whose English Wikipedia article carries no coordinates are rejected as concepts rather than places (OSM's `wikidata` tag sometimes points at a species, a saint or a unit of measurement instead of the building). Descriptions come from the **Wikipedia** article where one exists (593 of 654), the Wikidata one-liner otherwise (13), a category template only as a last resort (48). `avg_visit_minutes` and `price_level` remain heuristic by category — OSM doesn't reliably carry either — falling back to the OSM `fee` tag when present (80 rows) and to the Google Maps enrichment [`#71`](https://github.com/yukselburcinn-web/DA592/issues/71) added (60 more); `price_source` names which of the three every row took, and 514 still take the category default. 392 of 654 rows carry an `opening_hours` tag verbatim in `opening_hours_raw` — 197 read off OSM, 180 from the Google Maps enrichment [`#71`](https://github.com/yukselburcinn-web/DA592/issues/71), and 15 whose tag `hours_source` does not vouch for — and 358 of those name a day of the week, which the older `open_hour`/`close_hour` pair could not express at all ([`#70`](https://github.com/yukselburcinn-web/DA592/issues/70)); the pair remains as the coarse fallback for rows OSM never described. Every row records where each field came from (`description_source`, `hours_source`, `price_source`, plus `wikidata_qid` / `wikipedia_title` / `sitelink_count` / `monthly_pageviews`), and the column set is unchanged, so every downstream module keeps working unmodified. None of those provenance columns is read while a trip is planned — they are there so a reader can tell an observed value from a fallback, and so a rebuild can be compared against this one. That is deliberate; they are not dead weight to be trimmed.
...
The previous synthetic generators (`data/generate_data.py`) and the earlier single-source fetchers (`data/fetch_real_pois.py`, `data/fetch_real_demand.py`) are kept for history; they produce the old eight-city catalogue and are no longer what ships. **Running any of them overwrites shipped data with that eight-city set**, which is the half-migrated state described above — `generate_data.py` is the widest, writing `destinations.csv`, `poi.csv`, `transport.csv`, `demand_timeseries.csv` and `user_survey.csv` in one pass, and `fetch_real_demand.py` also reverts demand to the country-level `tour_occ_nim` proxy that `build_demand.py` replaced. `user_survey.csv` is the one file no pipeline script reproduces, so `generate_data.py` remains its source of record; regenerating it means restoring the other four afterwards (`git checkout` them).
