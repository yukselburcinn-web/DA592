...
- **Archetype-category precision shows a dramatic, unambiguous gap** (Fusion 0.964 vs. Hybrid 0.828 vs. Standard 0.453): only Graph-RAG has direct `ArchetypeProfile -[PREFERS]-> POI` edges, so only the fusion configuration systematically surfaces POIs matching the *traveler's segment* rather than just the query text. At 150 POIs/city there are enough genuinely off-archetype POIs in each city for this structural advantage to actually matter — at the old 10-POI scale, precision differences were muted simply because there wasn't much room to be wrong. Real embeddings (issue #4) widened Fusion's edge over Hybrid slightly further versus the TF-IDF+LSA baseline (was 0.946/0.875) — better semantic matches surface more *topically* relevant POIs for Hybrid too, but only Fusion's graph signal reliably matches the *archetype*, not just the topic.
- **Recall@k dropped in absolute terms versus the original 10-POI-per-city baseline (was 1.000/0.833) — this is expected, not a regression.** The "gold" multi-hop set is every POI of the queried category within 6km of a transport hub; at 150 POIs/city that gold set is now realistically large (often 15-20+ POIs), so a fixed `top_k=8` structurally caps recall well below 1.0 for *any* retriever. At the old scale the gold set was so small that top_k=8 trivially covered it, which is why the metric saturated near 1.0 and barely separated the two configs — a ceiling effect, not evidence Fusion and Hybrid were close in quality. Real embeddings then lifted recall for both configs versus the TF-IDF+LSA baseline (Fusion 0.368→0.479, Hybrid 0.327→0.390) — better semantic ranking recovers more of the gold set within the same `top_k=8` budget — and Fusion still leads Hybrid throughout, which is the ranking-quality question this metric is meant to test.
...
- **Automated tests**: `pytest roamwise/tests/ -v` — 21/21 passing, including the LangGraph-orchestrator equivalence test (auto-skipped via `pytest.importorskip` only if `requirements-langgraph.txt` isn't installed; CI installs it, so the GitHub Actions run genuinely exercises both orchestrator implementations on every push, not just the one bundled in `requirements.txt`). Covers graph construction/traversal, forecasting output shape, segmentation correctness, POI zoning completeness, all three retrieval configs, time-budget-respecting routing, day balancing and travel modes, full orchestrator execution (both implementations), and the comparative-analysis ordering claims above. The day-balancing tests are paired with a control (`test_no_fill_pass_reproduces_the_old_starved_day`) that switches the fix off and asserts the old empty day comes back, so they demonstrably fail for the right reason rather than passing incidentally.
